...
(cl:defmethod cl:initialize-instance :after ((m <Helm>) cl:&rest args)
  ...
  (cl:unless (cl:typep m 'Helm)
    (roslisp-msg-protocol:msg-deprecation-warning "using old message class name kingfisher_msgs-msg:<Helm> is deprecated: use kingfisher_msgs-msg:Helm instead.")))

(cl:ensure-generic-function 'thrust-val :lambda-list '(m))
(cl:defmethod thrust-val ((m <Helm>))
  (roslisp-msg-protocol:msg-deprecation-warning "Using old-style slot reader kingfisher_msgs-msg:thrust-val is deprecated.  Use kingfisher_msgs-msg:thrust instead.")
  (thrust m))

(cl:ensure-generic-function 'yaw_rate-val :lambda-list '(m))
(cl:defmethod yaw_rate-val ((m <Helm>))
  (roslisp-msg-protocol:msg-deprecation-warning "Using old-style slot reader kingfisher_msgs-msg:yaw_rate-val is deprecated.  Use kingfisher_msgs-msg:yaw_rate instead.")
  (yaw_rate m))
...
(cl:defmethod roslisp-msg-protocol:message-definition ((type (cl:eql '<Helm>)))
  ...
  (cl:format cl:nil "# Command a percentage amount of total thrust capacity, and an turn rate.~%# On a conventional craft, turn rate would map to rudder.~%~%# Thrust amount ranges from [-1.0..1.0], where 1.0 pushes Kingfisher forward.~%float32 thrust~%~%# Yaw rate specified in radians/sec, where positive values cause Kingfisher~%# to turn toward the port deck. The controller will use feedback from the~%# IMU's gyroscopes to attempt to match the command rate.~%float32 yaw_rate~%~%~%~%"))
(cl:defmethod roslisp-msg-protocol:message-definition ((type (cl:eql 'Helm)))
  "Returns full string definition for message of type 'Helm"
  (cl:format cl:nil "# Command a percentage amount of total thrust capacity, and an turn rate.~%# On a conventional craft, turn rate would map to rudder.~%~%# Thrust amount ranges from [-1.0..1.0], where 1.0 pushes Kingfisher forward.~%float32 thrust~%~%# Yaw rate specified in radians/sec, where positive values cause Kingfisher~%# to turn toward the port deck. The controller will use feedback from the~%# IMU's gyroscopes to attempt to match the command rate.~%float32 yaw_rate~%~%~%~%"))
